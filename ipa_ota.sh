...
if [[ ! ( # any of the following are not true
    ...
    then
        echo "    Usage: $(basename "$0") Application.ipa https://dl.dropbox.com/u/310759/iphone_hosting_base_URL/"
    exit;
fi
...
ver=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' Info.plist)
...
/usr/libexec/PlistBuddy -c "Set :items:0:metadata:bundle-version $ver" "${BASE}.plist"
/usr/libexec/PlistBuddy -c "Set :items:0:metadata:title $(sed "s/'/\\\\'/"<<<$name)" "${BASE}.plist"
if [[ ! -t 1 ]]; then
cat << HTML
<html>
